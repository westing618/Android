...
import com.ztd.yyb.util.DisplayUtil;
	...
	public void setImgUrlData(List<String> urlList) {
		...
		if (mUrlList != null && !mUrlList.isEmpty()) {
			...
			for (int i = 0; i < mUrlList.size(); i++) {
				...
				if (i == 0) {
					dotIv.setBackgroundResource(R.mipmap.jshop_banner_point_active);
				} else {
					dotIv.setBackgroundResource(R.mipmap.jshop_banner_point_inactive);
				}
				// 设置点的间距
...
//		mAutoRollRunnable.stop();
	...
	public void onPageSelected(int position) {
		dotList.get(prePosition).setBackgroundResource(
				R.mipmap. jshop_banner_point_inactive);
		dotList.get(position % dotList.size()).setBackgroundResource(
				R.mipmap.jshop_banner_point_active);
